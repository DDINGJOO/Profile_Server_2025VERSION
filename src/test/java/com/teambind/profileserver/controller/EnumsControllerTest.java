 ...
 *
 * <p>테스트 전략: 1. @SpringBootTest로 전체 Application Context 로드 2. InitTableMapper가 @PostConstruct에서
 * data.sql 데이터 로드 확인 3. MockMvc를 활용한 HTTP 요청/응답 검증 4. 반환되는 Map 구조 및 데이터 검증 5. HTTP 메서드 검증
 */
@SpringBootTest
...
class EnumsControllerTest {
  ...
  class GetInstruments {
    ...
    void getInstruments_Success() throws Exception {
      // when & then
      mockMvc
          .perform(get(BASE_URL + "/instruments"))
          .andDo(print())
          .andExpect(status().isOk())
          .andExpect(content().contentType(MediaType.APPLICATION_JSON))
          .andExpect(jsonPath("$").isMap());
    ...
    void getInstruments_WrongMethod_Post_Fail() throws Exception {
      // when & then
      mockMvc
          .perform(post(BASE_URL + "/instruments"))
          .andDo(print())
          .andExpect(status().isMethodNotAllowed());
    }
    ...
    void getInstruments_WrongMethod_Put_Fail() throws Exception {
      // when & then
      mockMvc
          .perform(put(BASE_URL + "/instruments"))
          .andDo(print())
          .andExpect(status().isMethodNotAllowed());
    }
  }
  ...
  class GetLocations {
    ...
    void getLocations_Success() throws Exception {
      // when & then
      mockMvc
          .perform(get(BASE_URL + "/locations"))
          .andDo(print())
          .andExpect(status().isOk())
          .andExpect(content().contentType(MediaType.APPLICATION_JSON))
          .andExpect(jsonPath("$").isMap());
    ...
    void getLocations_WrongMethod_Post_Fail() throws Exception {
      // when & then
      mockMvc
          .perform(post(BASE_URL + "/locations"))
          .andDo(print())
          .andExpect(status().isMethodNotAllowed());
    }
    ...
    void getLocations_WrongMethod_Put_Fail() throws Exception {
      // when & then
      mockMvc
          .perform(put(BASE_URL + "/locations"))
          .andDo(print())
          .andExpect(status().isMethodNotAllowed());
    }
  }
