 ...
 *
 * <p>테스트 전략: 1. @SpringBootTest로 전체 Application Context 로드 2. MockMvc를 활용한 HTTP 요청/응답 검증 3. Service
 * 계층은 MockBean으로 격리 4. 다양한 검색 조건 테스트 5. 커서 기반 페이징 검증 6. 배치 조회 기능 검증
 */
@SpringBootTest
...
class ProfileSearchControllerTest {
  ...
  class GetProfileById {
    ...
    void getProfile_Success() throws Exception {
      ...
      when(profileSearchService.searchProfileById(eq(TEST_USER_ID))).thenReturn(mockResponse);

      // when & then
      mockMvc
          .perform(get(BASE_URL + "/{userId}", TEST_USER_ID))
          .andDo(print())
    ...
    void getProfile_UserNotFound_Fail() throws Exception {
      ...
      // when & then
      mockMvc
          .perform(get(BASE_URL + "/{userId}", "nonExistentUser"))
          .andDo(print())
  ...
  class SearchProfiles {
    ...
    void searchProfiles_NoFilters_Success() throws Exception {
      ...
      when(profileSearchService.searchProfilesByCursor(
              any(ProfileSearchCriteria.class), any(), anyInt()))
    ...
    void searchProfiles_ByCity_Success() throws Exception {
      ...
      // when & then
      mockMvc.perform(get(BASE_URL).param("city", "서울")).andDo(print()).andExpect(status().isOk());
    }
    ...
    void searchProfiles_ByNickname_Success() throws Exception {
      ...
      // when & then
      mockMvc
          .perform(get(BASE_URL).param("nickName", "testNick"))
          .andDo(print())
          .andExpect(status().isOk());
    }
    ...
    void searchProfiles_ByGenres_Success() throws Exception {
      ...
      // when & then
      mockMvc
          .perform(get(BASE_URL).param("genres", "1", "2", "3"))
          .andDo(print())
          .andExpect(status().isOk());
    }
    ...
    void searchProfiles_ByInstruments_Success() throws Exception {
      ...
      // when & then
      mockMvc
          .perform(get(BASE_URL).param("instruments", "1", "2"))
          .andDo(print())
          .andExpect(status().isOk());
    }
    ...
    void searchProfiles_BySex_Success() throws Exception {
      ...
      // when & then
      mockMvc.perform(get(BASE_URL).param("sex", "M")).andDo(print()).andExpect(status().isOk());
    }
    ...
    void searchProfiles_WithCursorAndSize_Success() throws Exception {
      ...
      when(profileSearchService.searchProfilesByCursor(
              any(ProfileSearchCriteria.class), eq("cursor123"), eq(20)))
          .thenReturn(mockSlice);

      // when & then
      mockMvc
          .perform(get(BASE_URL).param("cursor", "cursor123").param("size", "20"))
          .andDo(print())
          .andExpect(status().isOk());
    }
    ...
    void searchProfiles_DefaultSize_Success() throws Exception {
      ...
      when(profileSearchService.searchProfilesByCursor(
              any(ProfileSearchCriteria.class), any(), eq(10)))
          .thenReturn(mockSlice);
    ...
    void searchProfiles_MaxSizeLimit_Success() throws Exception {
      ...
      when(profileSearchService.searchProfilesByCursor(
              any(ProfileSearchCriteria.class), any(), eq(100)))
          .thenReturn(mockSlice);

      // when & then
      mockMvc.perform(get(BASE_URL).param("size", "150")).andDo(print()).andExpect(status().isOk());
    }
    ...
    void searchProfiles_BlankParams_Success() throws Exception {
      ...
      // when & then
      mockMvc
          .perform(get(BASE_URL).param("city", "").param("nickName", "   ").param("cursor", ""))
          .andDo(print())
          .andExpect(status().isOk());
    }
    ...
    void searchProfiles_EmptyResult_Success() throws Exception {
      ...
      // when & then
      mockMvc
          .perform(get(BASE_URL).param("city", "존재하지않는도시"))
          .andDo(print())
          .andExpect(status().isOk())
  ...
  class GetProfilesBatch {
    ...
    void getProfilesBatch_Success() throws Exception {
      ...
      BatchUserSummaryResponse response1 =
          BatchUserSummaryResponse.builder()
              .userId("user1")
              .nickname("nick1")
              .profileImageUrl("https://example.com/1.jpg")
              .build();

      BatchUserSummaryResponse response2 =
          BatchUserSummaryResponse.builder()
              .userId("user2")
              .nickname("nick2")
      ...
      BatchUserSummaryResponse response3 =
          BatchUserSummaryResponse.builder()
              .userId("user3")
              .nickname("nick3")
      ...
      when(profileSearchService.searchProfilesByIds(eq(userIds))).thenReturn(mockResponses);
    ...
    void getProfilesBatch_SingleUser_Success() throws Exception {
      ...
      BatchUserSummaryResponse response =
          BatchUserSummaryResponse.builder()
              .userId("user1")
              .nickname("nick1")
              .profileImageUrl("https://example.com/1.jpg")
      ...
      when(profileSearchService.searchProfilesByIds(eq(userIds))).thenReturn(List.of(response));
    ...
    void getProfilesBatch_EmptyList_Success() throws Exception {
      ...
      when(profileSearchService.searchProfilesByIds(eq(userIds))).thenReturn(List.of());
    ...
    void getProfilesBatch_NoContentType_Fail() throws Exception {
      ...
      // when & then
      mockMvc
          .perform(post(BASE_URL + "/batch").content(objectMapper.writeValueAsString(userIds)))
          .andDo(print())
