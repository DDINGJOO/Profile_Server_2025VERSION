 ...
 *
 * 테스트 전략:
 * 1. @SpringBootTest로 전체 Application Context 로드 (InitTableMapper 포함)
 * 2. MockMvc를 활용한 HTTP 요청/응답 검증
 * 3. Service 계층은 MockBean으로 격리
 * 4. JSON 직렬화/역직렬화 검증
 * 5. 검증 어노테이션 동작 확인 (AttributeValidator가 InitTableMapper 참조)
 * 6. H2 in-memory DB 사용 (test 프로필)
 * 7. TestConfig로 Kafka Mock Bean 제공
 */
@SpringBootTest
...
class ProfileUpdateControllerTest {
	...
	@Autowired
	private com.teambind.profileserver.utils.InitTableMapper initTableMapper;
    ...
    class UpdateProfile {
        ...
        void updateProfile_UserNotFound_Fail() throws Exception {
            ...
            // when & then
            mockMvc.perform(put(BASE_URL + "/{userId}", "nonExistentUser")
                            .contentType(MediaType.APPLICATION_JSON)
                            .content(objectMapper.writeValueAsString(request)))
                    .andDo(print())
    ...
    class HttpMethodTests {
        ...
        void wrongHttpMethod_Post() throws Exception {
            // when & then
            mockMvc.perform(post(BASE_URL + "/{userId}", TEST_USER_ID)
                            .contentType(MediaType.APPLICATION_JSON)
        ...
        void wrongHttpMethod_Delete() throws Exception {
            // when & then
            mockMvc.perform(org.springframework.test.web.servlet.request.MockMvcRequestBuilders.delete(BASE_URL + "/{userId}", TEST_USER_ID))
                    .andDo(print())
                    .andExpect(status().isMethodNotAllowed());
        }
    }
