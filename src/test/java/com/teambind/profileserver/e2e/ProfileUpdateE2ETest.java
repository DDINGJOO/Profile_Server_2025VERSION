 ...
 *
 * 테스트 전략:
 * 1. API 엔드포인트부터 DB까지 전체 플로우 테스트
 * 2. 실제 HTTP 요청/응답 검증
 * 3. Controller → Service → Repository → DB 전체 흐름 확인
 * 4. Location 검증 로직 포함한 실제 사용 시나리오 테스트
 * 5. 성공/실패 케이스 모두 검증
 */
@SpringBootTest
...
class ProfileUpdateE2ETest {
    ...
    class LocationE2ETests {
        ...
        void updateCity_MultipleChanges_E2E() throws Exception {
            // given - 첫 번째 변경
            ProfileUpdateRequest request1 = ProfileUpdateRequest.builder()
                    .nickname("testNick1")
                    .city("BUSAN")
                    .chattable(true)
                    .publicProfile(true)
            ...
            // given - 두 번째 변경
            ProfileUpdateRequest request2 = ProfileUpdateRequest.builder()
                    .nickname("testNick2")
                    .city("DAEGU")
                    .chattable(true)
                    .publicProfile(true)
    ...
    class ExceptionE2ETests {
        ...
        void updateProfile_UserNotFound_Returns404() throws Exception {
            ...
            // when & then
            mockMvc.perform(put(BASE_URL + "/{userId}", nonExistentUserId)
                            .contentType(MediaType.APPLICATION_JSON)
                            .content(objectMapper.writeValueAsString(request)))
                    .andDo(print())
    ...
    class RealWorldScenarioE2ETests {
        ...
        void scenario_MusicianChangesGenreFromRockToJazz() throws Exception {
            // given - 초기: 록 장르
            ProfileUpdateRequest initialRequest = ProfileUpdateRequest.builder()
            ...
            // when - 재즈로 변경
            ProfileUpdateRequest changeRequest = ProfileUpdateRequest.builder()
