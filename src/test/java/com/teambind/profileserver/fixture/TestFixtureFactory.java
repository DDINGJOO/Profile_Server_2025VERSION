 ...
 * 테스트용 객체 생성 팩토리 클래스
 *
 * <p>목적: 1. 테스트 데이터 생성 로직 중앙화 2. 메인 코드 변경 시 테스트 수정 최소화 3. 테스트 가독성 향상
 */
public class TestFixtureFactory {
  ...
  public static GenreNameTable createGenre(int id, String name) {
    return GenreNameTable.builder().genreId(id).genreName(name).build();
  }
  ...
  public static InstrumentNameTable createInstrument(int id, String name) {
    return InstrumentNameTable.builder().instrumentId(id).instrumentName(name).build();
  }
  ...
  public static UserInstruments createUserInstrument(
      UserInfo userInfo, InstrumentNameTable instrument) {
