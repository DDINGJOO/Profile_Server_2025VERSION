 ...
 *
 * <p>테스트 전략: 1. Service → Repository → DB 전체 플로우 검증 2. DTO 변환 로직 검증 3. 실제 검색 시나리오 테스트 4. 예외 상황 처리
 * 검증
 */
@SpringBootTest
@ActiveProfiles("test")
...
class ProfileSearchIntegrationTest {
  ...
  void searchProfilesByIds_Success() {
    ...
    assertThat(result).extracting("userId").containsExactlyInAnyOrder("batch1", "batch2", "batch3");
  }
  ...
  class SearchProfilesTests {
    ...
    void searchProfiles_ByCity_Success() {
      ...
      assertThat(result.getContent()).allMatch(response -> response.getCity().equals("서울"));
    }
    ...
    void searchProfiles_ByNickname_PartialMatch_Success() {
      // given
      ProfileSearchCriteria criteria = ProfileSearchCriteria.builder().nickName("Rock").build();
      PageRequest pageable = PageRequest.of(0, 10);
  ...
  class SearchProfilesByCursorTests {
    ...
    void searchByCursor_NextPage_Success() {
      ...
      Slice<UserResponse> firstPage =
          profileSearchService.searchProfilesByCursor(criteria, null, 5);
      String cursor = firstPage.getContent().get(4).getUserId();

      // when
      Slice<UserResponse> secondPage =
          profileSearchService.searchProfilesByCursor(criteria, cursor, 5);
