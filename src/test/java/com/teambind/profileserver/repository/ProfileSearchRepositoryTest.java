 ...
 *
 * 테스트 전략:
 * 1. @SpringBootTest로 전체 Context 로드 (QueryDSL 설정 포함)
 * 2. 실제 DB 사용하여 QueryDSL 쿼리 검증
 * 3. 다양한 검색 조건 조합 테스트
 * 4. 페이징 및 커서 기반 페이징 테스트
 */
@SpringBootTest
@ActiveProfiles("test")
...
class ProfileSearchRepositoryTest {
    ...
    class SearchByCriteriaTests {
        ...
        void search_ByCity_Success() {
            ...
            assertThat(result.getContent())
                    .allMatch(user -> user.getCity().equals("SEOUL"));
        }
    ...
    class BatchSearchTests {
        ...
        void searchByUserIds_PartialExists_ReturnsExisting() {
            ...
            assertThat(result).extracting("userId")
                    .containsExactlyInAnyOrder("exists1", "exists2");
        }
    }
}
