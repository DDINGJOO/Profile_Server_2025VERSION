 ...
 *
 * <p>테스트 범위: 1. 사용자 프로필 생성 (createUserProfile) 2. 사용자 프로필 삭제 (deleteUserProfile)
 *
 * <p>테스트 전략: - ArgumentCaptor로 실제 저장되는 객체 검증 - 닉네임 생성 로직 검증 - 예외 상황 처리 확인
 */
...
class UserInfoLifeCycleServiceTest {
  ...
  @InjectMocks private UserInfoLifeCycleService userInfoLifeCycleService;
  ...
  class CreateUserProfile {
    ...
    void createUserProfile_Success() {
      ...
      assertThat(savedUser.getNickname().toLowerCase())
          .contains(TEST_PROVIDER); // provider가 닉네임에 포함됨
    }
    ...
    void createUserProfile_NicknameGenerated() {
      ...
      assertThat(savedUser.getNickname()).isNotNull().isNotBlank();
    }
  }
