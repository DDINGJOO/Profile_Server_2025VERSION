 ...
 *
 * <p>테스트 전략: 1. Mockito를 활용한 단위 테스트 2. Given-When-Then 패턴 사용 3. DisplayName으로 한글 명세 작성 4. Factory
 * 패턴으로 테스트 데이터 생성 5. 엣지 케이스 및 예외 상황 커버
 */
...
class ProfileUpdateServiceTest {
  ...
  @InjectMocks private ProfileUpdateService profileUpdateService;
  ...
  class UpdateBasicFields {
    ...
    void updateNickname_Duplicate_ThrowsException() {
      ...
      ProfileUpdateRequest request = updateRequest().nickname(duplicateNickname).build();
    ...
    void updateNickname_Same_NoEventPublished() {
      ...
      ProfileUpdateRequest request = updateRequest().nickname(sameNickname).build();
    ...
    void updateCity_Success() {
      // given
      testUser.setCity("SEOUL");
      ProfileUpdateRequest request = updateRequest().city("BUSAN").build();
    ...
    void updateCity_NullDoesNotChange() {
      // given
      testUser.setCity("SEOUL");
      ProfileUpdateRequest request = updateRequest().city(null).build();
    ...
    void updateCity_ChangeToAnotherCity_Success() {
      // given
      testUser.setCity("SEOUL");
      ProfileUpdateRequest request = updateRequest().city("DAEGU").build();
  ...
  class UpdateGenres {
    ...
    void addGenres_Success() {
      // given
      ProfileUpdateRequest request = updateRequest().genres(List.of(1, 2, 3)).build();
      ...
      List<Integer> genreIds =
          testUser.getUserGenres().stream().map(ug -> ug.getGenre().getGenreId()).toList();
  ...
  class UpdateInstruments {
    ...
    void addInstruments_Success() {
      // given
      ProfileUpdateRequest request = updateRequest().instruments(List.of(1, 2)).build();
      ...
      List<Integer> instrumentIds =
          testUser.getUserInstruments().stream()
  ...
  class ComplexScenarios {
    ...
    void updateMinimalFields_Success() {
      // given
      ProfileUpdateRequest request = updateRequest().nickname("minimalNick").build();
